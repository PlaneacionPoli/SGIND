...
| M-11 | ✅ **Hecho (Oleada 4)** — `ai_configs`/`ai_prompts` eliminadas (migración 003), `audit_log` conservada por decisión de negocio, modelo `Accion` eliminado | G-10, G-11 | — | — | — | — |
| M-12 | ✅ **Resuelto de facto** — los 4 scripts ya no están en `scripts/`, solo en `legacy-reference/scripts/`. Falta solo decidir si `legacy-reference/` se conserva como referencia histórica | G-11 | — | — | — | — |
...
| M-14 | ✅ **Convención documentada (Oleada 4)** — Plotly para vistas analíticas completas, Recharts para widgets en modales/tarjetas (`01-arquitectura.md`). Migrar a una sola librería sigue siendo opción futura | G-14 | Reduce ambigüedad para código nuevo | — | — | **Evolución futura (solo unificación)** |
