...
| M-06 | Consolidar la semaforización: eliminar RN-04/05/06/07 (backend) y los 4 mapas de color (frontend), dejando `domain/categorization.py` como única fuente y un único módulo de color en frontend que consuma el `nivel`/`color` que ya manda el backend | Riesgo crítico de inconsistencia visual entre pantallas (G-03) | Elimina la causa raíz de una clase entera de bugs de UI | Medio-alto (toca varios archivos, requiere regresión visual) | Medio | **Mejora estructural** |
...
| M-08 | Añadir alerta o indicador de "última actualización" visible en el dashboard, basada en la fecha de modificación real de `Resultados Consolidados.xlsx` | Hoy no hay forma de que el usuario final sepa si los datos están desactualizados (consecuencia de G-01) | Mitiga el riesgo de G-01 mientras se implementa M-07 | Bajo | Bajo | **Quick win, mitigación temprana de M-07** |
