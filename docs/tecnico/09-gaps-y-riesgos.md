...
| G-08 | Documentación de migración (`STATUS.md`/`ROADMAP.md`) describe una estructura `sgind-v2/*` que no existe en el repo | Repo real tiene `backend/`/`frontend/` en raíz | Cualquier persona que siga esos documentos literalmente no encuentra los archivos |
| G-09 | `PLAN_MIGRACION_PRIORIZADO.md` (2026-09-18) ya está parcialmente desactualizado dos días después | Módulo Plan de Mejoramiento (Indicadores+Métricas) que decía "sin ningún equivalente en SGING" ya está implementado en `backend/app/domain/plan_mejoramiento_builders.py` | Riesgo de tomar decisiones de priorización sobre un hallazgo que ya no es cierto |
...
| G-13 | Conteo de ADRs incorrecto en `ROADMAP.md` (dice 8, hay 9) | `Glob docs/architecture/adrs/*` | Menor, solo corrección editorial |
...
| G-16 | Dos páginas completas y funcionales (PDI/Acreditación, Diagnóstico) sin ningún enlace de navegación visible | `BETA_ITEMS`/`BETA_ITEM_META` en `config/navigation.ts` no se usan en `Sidebar.tsx` ni `LauncherScreen.tsx` | Funcionalidad terminada e invisible para el usuario final salvo que conozca la URL |
| G-17 | Texto fijo engañoso en panel de Diagnóstico: `"Auth: Dev login activo (Azure AD en Fase 7)"` | `diagnostico/page.tsx:181` | No refleja el modo de auth realmente activo en tiempo de ejecución |
