...
def main() -> int:
    ...
    # Nota: scripts/plan_mejoramiento/build_catalogo_indicadores.py NO se incluye
    # aqui a proposito. Genera un borrador heuristico de Signo/Decimales que su
    # propio docstring exige revisar fila por fila antes de usar en produccion,
    # y ya alimenta calculos reales del backend (plan_mejoramiento_builders.py).
    # Decision de negocio (2026-09-20): solo se ejecuta manualmente, a criterio
    # humano, cuando aparece una hoja nueva en el Anexo Estadistico que requiera
    # mapeo -- nunca en el cron automatico, para no arriesgar sobrescribir
    # correcciones manuales ya hechas.
    ...
    try:
        ...
        qa["enabled"] = False
        _append_text(log_path, f"\n[QA] Fallo inesperado, QA deshabilitado:\n{traceback.format_exc()}\n")
    ...
    if avisos:
        print(f"[AVISO] {len(avisos)} aviso(s) requieren revision manual (ver report.avisos):")
        for a in avisos:
            print("  -", a)
